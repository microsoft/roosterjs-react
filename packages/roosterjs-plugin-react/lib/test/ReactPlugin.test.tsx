...
const ExampleComponent = (props: Partial<ReactPluginComponentProps>) => {
    const [counter, setCounter] = React.useState(() => (props.initialSerializedSharableState ? JSON.parse(props.initialSerializedSharableState).counter : 0));
...
describe("ReactPlugin", () => {
    ...
    beforeEach(() => {
        globalShouldUpdateState = false;
        editor = TestHelper.initEditor("ReactPlugin", [new ReactPlugin("example-component", ExampleComponent)]);
    });
    ...
    describe("when a new mountpoint is added to the document", () => {
        ...
        it("renders a component into the mountpoint", () => {
            ...
            expect(queriedElements[0].innerHTML).toEqual('<div class="example-component">Counter: 0</div>');
        });
        ...
        it("renders updates from the component into the mountpoint", () => {
            ...
            const queriedElements = editor.queryElements('[data-rcp-compid="example-component"]');
            expect(queriedElements[0].innerHTML).toEqual('<div class="example-component">Counter: 10</div>');
        });
    });
    ...
    describe("when a mountpoint is moved from one part of the document to another", () => {
        it("renders the component into the new mountpoint", () => {
            // Arrange
            editor.insertContent('<div data-rcp-compid="example-component"></div> Hello kids, it is me <div data-select-target="moveto"></div>');
            editor.triggerContentChangedEvent();
            const initialMountPoints = editor.queryElements('[data-rcp-compid="example-component"]');
            ...
            expect(moveToTarget.children[0].innerHTML).toEqual('<div class="example-component">Counter: 113</div>');
        });
        ...
        it("Uses the same component instance, preserving state", () => {
            ...
            expect(moveToTarget.children[0].innerHTML).toEqual('<div class="example-component">Counter: 123</div>');
        });

        it("Makes the new mountpoint contenteditbale=false", () => {
            // Arrange
            editor.insertContent('<div data-rcp-compid="example-component"></div> Hello kids, it is me <div data-select-target="moveto"></div>');
            editor.triggerContentChangedEvent();
            const initialMountPoints = editor.queryElements('[data-rcp-compid="example-component"]');
            ...
            // Assert
            expect(moveToTarget.children[0].getAttribute("contenteditable")).toBe("false");
        });
    });
    ...
    describe("when a duplicate mountpoint is copied and pasted into the document", () => {
        it("creates a new ID for the mountpoint and sets it on the document", () => {
            ...
            expect(initialMountPoint.innerHTML).toEqual('<div class="example-component">Counter: 12</div>');
            // check it has its own target
            expect(copyToTarget.children[0].getAttribute("data-rcp-instid")).not.toBeFalsy();
            expect(copyToTarget.children[0].getAttribute("data-rcp-instid")).not.toEqual(initialMountPoint.getAttribute("data-rcp-instid"));
            expect(copyToTarget.children[0].innerHTML).toEqual('<div class="example-component">Counter: 0</div>');
        });
        it("recognizes the original mountpoint, even if it is 2nd in document order", () => {
            // Arrange
            editor.insertContent('Hello kids, it is me <div data-select-target="moveto"></div> This is the initial mount point: <div data-rcp-compid="example-component"></div>');
            editor.triggerContentChangedEvent();
            const initialMountPoints = editor.queryElements('[data-rcp-compid="example-component"]');
            expect(initialMountPoints.length).toBe(1);
            const initialMountPoint = initialMountPoints[0];
    ...
    describe("when a component has sharable state", () => {
        it("initializes the component with that sharable state", () => {
            // Act
            editor.insertContent('<div data-rcp-compid="example-component" data-rcp-st="{&quot;counter&quot;: 888}"></div>');
            editor.triggerContentChangedEvent();
...
describe("interaction with darkmode", () => {
    ...
    describe("in lightmode", () => {
        ...
        it("renders with normal colors outside of darkmode", () => {
            ...
            const mountPoint = editor.queryElements('[data-rcp-compid="example-colour-component"]')[0];
