...
import { ChangeSource, PluginEvent, PluginEventType, ContentPosition, ExtractContentEvent } from "roosterjs-editor-types";
import { REACT_COMPONENT_DATA_KEY, REACT_COMPONENT_INSTANCE_ID, REACT_COMPONENT_SHARABLE_STATE } from "./util/constants";
...
export default class ReactComponentPlugin implements EditorPlugin {
    ...
    constructor(componentId: string, componentType: React.ComponentType<ReactPluginComponentProps>) {
    ...
    private handleChangeInEditor(event: PluginEvent) {
        ...
        const foundComponentRoots = this.editor.queryElements(`[${REACT_COMPONENT_DATA_KEY}="${this.componentId}"]`);
    ...
    private moveMountedComponentToNewPositionInEditor(element: HTMLElement, oldInstance: MountedComponentInstance) {
    ...
    private mountNewInstanceInEditor(newInstance: MountedComponentInstance) {
        ...
        newInstance.inEditorMountRoot.setAttribute(REACT_COMPONENT_INSTANCE_ID, newInstance.instanceId);
    ...
    private mountInstanceOnShadowDom(instanceToMount: MountedComponentInstance) {
        ...
        const serializedState = instanceToMount.inEditorMountRoot.getAttribute(REACT_COMPONENT_SHARABLE_STATE);
        const updateSerializedSharableStateOnDom = (newState: string) => {
            instanceToMount.inEditorMountRoot.setAttribute(REACT_COMPONENT_SHARABLE_STATE, newState);
        };
        ...
        const onRenderCallback = () => {
            ...
            if (shadowRootClone.childNodes.length !== 1) {
                throw new Error("ReactPlugin only supports components that render a single root element");
            }
    ...
    private isTriggeringEvent(event: PluginEvent): boolean {
        if (event.eventType === PluginEventType.EditorReady) {
            return true;
        } else if (event.eventType == PluginEventType.ContentChanged && legalChangeSources.has(event.source)) {
            return true;
        }
        return false;
    }
