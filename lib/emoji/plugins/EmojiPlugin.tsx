...
import { cacheGetCursorEventData, replaceTextBeforeCursorWithNode, clearCursorEventDataCache } from 'roosterjs-editor-api';
...
export default class EmojiPlugin implements EditorPlugin {
    ...
    private onKeyUpSuggestingDomEvent(event: PluginDomEvent) {
        ...
        if ((keyboardEvent.key.length == 1 && keyboardEvent.which != Key.Space) || keyboardEvent.which == Key.Backspace) {
    ...
    private onKeyUpDomEvent(event: PluginDomEvent) {
        ...
        let keyboardEvent = event.rawEvent as KeyboardEvent;
        let wordBeforeCursor = this.getWordBeforeCursor(event)
        if ((keyboardEvent.which == Key.Colon || keyboardEvent.which == Key.ColonFirefox) && wordBeforeCursor == ':') {
    ...
    private getWordBeforeCursor(event: PluginEvent): string {
        ...
        return matches && matches.length > 2 && matches[0] == wordBeforeCursor
            ? matches[2]
            : null;
    }
